...
1. **Pull** the item from `/mnt/d/Obsidian/vaults/BiteVault/Backlog.md` (and `Tasks/$ARGUMENTS.md` if it exists).
...
3. **Discuss** the approach with me before building. **HARD CHECKPOINT — do not write any code yet.**
...
6. **Branch** off `develop`, named `$ARGUMENTS/<short-description>`.
...
9. **Close the loop in the vault** — mark the item `Done` in `Backlog.md`, update `Progress.md`, write `Fixes/$ARGUMENTS.md` and any `Decisions/` record, append what shipped to the session note.
